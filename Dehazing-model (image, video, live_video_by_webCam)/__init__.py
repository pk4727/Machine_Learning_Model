...
class image_dehazer():
    ...
    def __BoundCon(self, HazeImg):
        if (len(HazeImg.shape) == 3):
            t_b = np.maximum((self._A[0] - HazeImg[:, :, 0].astype(float)) / (self._A[0] - self.C0), (HazeImg[:, :, 0].astype(float) - self._A[0]) / (self.C1 - self._A[0]))
            t_g = np.maximum((self._A[1] - HazeImg[:, :, 1].astype(float)) / (self._A[1] - self.C0), (HazeImg[:, :, 1].astype(float) - self._A[1]) / (self.C1 - self._A[1]))
            t_r = np.maximum((self._A[2] - HazeImg[:, :, 2].astype(float)) / (self._A[2] - self.C0), (HazeImg[:, :, 2].astype(float) - self._A[2]) / (self.C1 - self._A[2]))
            MaxVal = np.maximum(t_b, t_g, t_r)
            self._Transmission = np.minimum(MaxVal, 1)
        else:
            self._Transmission = np.maximum((self._A[0] - HazeImg.astype(float)) / (self._A[0] - self.C0), (HazeImg.astype(float) - self._A[0]) / (self.C1 - self._A[0]))
    ...
    def __CalculateWeightingFunction(self, HazeImg, Filter):
        # Computing the weight function... Eq(17)
    ...
    def __CalTransmission(self, HazeImg):
        ...
        for i in range(len(KirschFilters)):
            ...
            DS = DS + (abs(D) ** 2)
        # Cyclic loop for refining t and u --> Section III
        beta = 1  # Start Beta value
        beta_max = 2 ** 4  # Selected from --> Section III --> "Scene Transmission Estimation"
        ...
        while (beta < beta_max):
            ...
            # Fixing u and solving t --> eq(26)
            # Note: In eq(26), the Numerator is the "DU" calculated in the above part of the code
            # In the eq(26), the Denominator is the DS which was computed as a constant in the above code
    ...
    def __psf2otf(self, psf, shape):
        ...
        # Compute the OTF
        otf = np.fft.fft2(psf)
        # Estimate the rough number of operations involved in the FFT and discard the PSF imaginary part if within roundoff error
        # roundoff error  = machine epsilon = sys.float_info.epsilon or np.finfo().eps
    ...
    def __zero_pad(self, image, shape, position='corner'):
        ...
        Returns
        -------
        padded_img: real `numpy.ndarray' The zero-padded image
        """
...
def remove_haze(HazeImg, airlightEstimation_windowSze=15, boundaryConstraint_windowSze=3, C0=20, C1=300, regularize_lambda=0.1, sigma=0.5, delta=0.85, showHazeTransmissionMap=True):
    Dehazer = image_dehazer(airlightEstimation_windowSze=airlightEstimation_windowSze, boundaryConstraint_windowSze=boundaryConstraint_windowSze, C0=C0, C1=C1,
                            regularize_lambda=regularize_lambda, sigma=sigma, delta=delta, showHazeTransmissionMap=showHazeTransmissionMap)
